...
**34 tables** in `packages/db/src/schema.ts`, one of which is `schema_migrations`; 30 migrations.
...
as CI measured them on 2026-08-26. The unit figure has twice been stale here by more than a hundred — treat any
...
from a run that only sees what is committed. Those three are committed now and the two counts agree
again — which is what makes the rule easy to forget. The
